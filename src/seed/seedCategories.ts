...
const rawCategoriesData = fs.readFileSync('src/seed/categories.json', 'utf-8');
  ...
  try {
    for (const [parentCategoryName, subcategories] of Object.entries(categoriesData)) {
      ...
      for (const [subcategoryName, details] of Object.entries(subcategories)) {
        ...
        for (const specName of details.Specifications) {
          const spec = new Specification({ name: specName });
          await spec.save();
